...
test("unsigned development builds do not offer automatic desktop updates", async () => {
  const [updater, settings, rust, capability, permissions] = await Promise.all([
    source("lib/desktop-updater.ts"),
    source("components/AppSettings.tsx"),
    source("src-tauri/src/lib.rs"),
    source("src-tauri/capabilities/desktop-updater.json"),
    source("src-tauri/permissions/desktop-shell.toml"),
  ]);

  assert.match(updater, /invoke<boolean>\("is_desktop_updater_available"\)/);
  assert.match(settings, /updaterAvailable === true/);
  assert.match(settings, /updaterAvailable === false/);
  assert.match(rust, /fn is_desktop_updater_available\(\) -> bool/);
  assert.match(rust, /is_desktop_updater_available,/);
  assert.match(capability, /allow-is-desktop-updater-available/);
  assert.match(permissions, /commands\.allow = \["is_desktop_updater_available"\]/);
});
